...
use crate::wasm::{ModuleLoader, ModuleCid};
...
impl ContentResolver {
    ...
    pub fn new(network: Arc<Mutex<NetworkNode>>, loader: Arc<ModuleLoader>) -> Self {
        Self { network, loader }
    }
    ...
    async fn resolve_name(&self, name: &str) -> Result<Option<String>> {
        ...
        if let Some(module_info) = results.first() {
            debug!("Resolved '{}' to CID: {}", name, module_info.cid);
            Ok(Some(module_info.cid.0.clone()))
        } else {
            Ok(None)
        }
    }
    ...
    pub async fn is_available(&self, cid: &str) -> bool {
        ...
        let mut network = self.network.lock().await;
        if let Ok(Some(_metadata)) = network.find_module_by_cid(&module_cid).await {
    ...
    pub async fn fetch_module(&self, cid: &str) -> Result<Option<Vec<u8>>> {
        ...
        if let Ok(Some(_metadata)) = network.find_module_by_cid(&module_cid).await {
            ...
            // TODO: Implement full peer-to-peer fetch
            // For now, return None to indicate network fetch not complete
            warn!("Network fetch not fully implemented yet");
            return Ok(None);
        }
        
        warn!("Module {} not found in network", cid);
        Ok(None)
    }
}
