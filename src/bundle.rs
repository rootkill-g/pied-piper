    ...
    pub async fn new(
        ...
        if let Some(assets_dir) = assets_dir {
            ...
            if assets_dir.exists() {
                Self::load_assets_recursive(assets_dir, assets_dir, &mut assets, &mut asset_paths).await?;
                info!("Loaded {} assets", assets.len());
            }
        }
        
        let total_size = wasm_module.len() + assets.values().map(|v| v.len()).sum::<usize>();
        let entry_point = wasm_path
            .file_name()
            .unwrap()
            .to_string_lossy()
            .to_string();
...
mod tests {
    ...
    fn test_content_type_detection() {
        ...
        assert_eq!(AppBundle::content_type_for_path("app.js"), "application/javascript");
        assert_eq!(AppBundle::content_type_for_path("data.json"), "application/json");
        assert_eq!(AppBundle::content_type_for_path("module.wasm"), "application/wasm");
    }
}
