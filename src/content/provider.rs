...
impl ModuleProvider {
    ...
    pub async fn provide_module(&self, info: ModuleInfo, bytes: Arc<Vec<u8>>) -> Result<()> {
    ...
    pub async fn handle_request(&self, request: ModuleRequest) -> ModuleResponse {
        match request {
            ModuleRequest::GetModule { cid } => self.handle_get_module(&cid).await,

            ModuleRequest::GetModuleInfo { cid } => self.handle_get_module_info(&cid).await,

            ModuleRequest::SearchByName { name } => self.handle_search_by_name(&name).await,

            ModuleRequest::ListModules => self.handle_list_modules().await,
        }
    }
    ...
    async fn handle_get_module_info(&self, cid: &str) -> ModuleResponse {
        ...
        match info_map.get(cid) {
            Some(info) => ModuleResponse::ModuleInfo {
